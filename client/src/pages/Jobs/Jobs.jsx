  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-5">
        ...
        <div className=''>
          {jobData.map((job) => {
            return <div key={job._id} className='p-5 border-gray-300 border rounded-md mt-5 flex flex-col gap-3'>
            <div className='flex'>
              <h2 className='font-medium text-xl'>{job.title}</h2>
              <span className=' bg-blue-200 text-blue-800 rounded-full text-xs text-center inline-flex items-center justify-center px-5'>{job.category}</span>
            </div>
            ...
            <div className='flex gap-6'>
              {job.requiredSkills.slice(0,3).map((skill) => {
                return <span key={skill} className='bg-gray-200 p-1 rounded-full text-sm text-gray-600 '>{skill}</span>
              })}
            </div>
            <div className='flex gap-4 pt-5'>
              <button className='px-14 py-2 bg-blue-700 text-white text-lg font-light rounded-lg'>Apply Now</button>
              <button className='px-4 py-2 border text-gray-500 rounded-lg '> View Details</button>
            </div>
          </div>
          })}
