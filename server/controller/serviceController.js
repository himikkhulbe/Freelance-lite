...
export const getOrders = async (req, res) => {
    const userId = req.user._id;
    try {
        const orders = await Order.find({ client: userId }).populate("freelancer", "name contactInfo location").populate("service", "title description price deliveryTime revisions").sort({ createdAt: -1 });
